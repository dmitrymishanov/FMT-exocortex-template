...
3. **Sync Gate (актуализация контекста РП).** Прочитать контекст РП и связанных РП → синхронизировать открытые фазы с тем, что фактически сделано.
   - **EXTENSION POINT (protocol-open sync):** `bash .claude/scripts/load-extensions.sh protocol-open sync` — exit 0 → `Read` каждый файл из вывода (alphabetic) → выполнить. Exit 1 → пропустить (нет L3 кастомизации, базовое поведение: только прочитать `inbox/WP-{N}-*.md`).
   - Поддерживает `extensions/protocol-open.sync.md` И `extensions/protocol-open.sync.<suffix>.md`.
   - Цель: исключить дублирование работы и ложные блокеры на момент Ритуала. Если sync обнаружил противоречие («PASS» в одном связанном vs «FAIL» в другом по той же метрике) — НЕ применять автоматически, поднять в «Требует внимания» Ритуала.
4. → Ритуал.
